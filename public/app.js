...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const TRANSLATIONS = {
    vi: {
      ...
      changelog_title_v170: "🤖 Scraper Miễn Phí + Bộ Lọc Ảnh AI + Infinite Scroll",
      changelog_li1_v170: "<span class=\"highlight-cyan\">DuckDuckGo & Bing Scraper:</span> Thêm module scrapers.js cào ảnh nón hoàn toàn miễn phí, không cần API key, hỗ trợ phân trang vô hạn qua DuckDuckGo (VQD token) và Bing (HTML parse).",
      changelog_li2_v170: "<span class=\"highlight-cyan\">Tích Hợp Firecrawl:</span> Thêm firecrawl_scraper.js sử dụng Firecrawl API làm overlay bổ sung ảnh nón từ Pinterest và các nguồn thời trang.",
      changelog_li3_v170: "<span class=\"highlight-cyan\">Bộ Lọc Ảnh AI (Gemini LLM):</span> Tích hợp filterHatsWithLLM dùng Gemini 1.5 Flash tự động phát hiện và loại bỏ ảnh rác (logo, quảng cáo, banner) không liên quan đến nón/mũ/thời trang trước khi lưu cache.",
      changelog_li4_v170: "<span class=\"highlight-cyan\">Infinite Scroll Mở Rộng:</span> Feed trang chủ hỗ trợ tải trang vô hạn, tự động gọi thêm kết quả khi cuộn đến cuối, pipeline cào ưu tiên: Cache → DuckDuckGo → Bing → Serper → Firecrawl.",
    ...
    en: {
      ...
      changelog_title_v170: "🤖 Free Scrapers + AI Image Filter + Infinite Scroll",
      changelog_li1_v170: "<span class=\"highlight-cyan\">DuckDuckGo & Bing Scraper:</span> Added scrapers.js module for completely free hat image scraping (no API key needed), with infinite pagination via DuckDuckGo (VQD token) and Bing (HTML parse).",
      changelog_li2_v170: "<span class=\"highlight-cyan\">Firecrawl Integration:</span> Added firecrawl_scraper.js using the Firecrawl API as an overlay to supplement hat images from Pinterest and fashion sources.",
      changelog_li3_v170: "<span class=\"highlight-cyan\">AI Image Filter (Gemini LLM):</span> Integrated filterHatsWithLLM using Gemini 1.5 Flash to automatically detect and remove garbage images (logos, ads, banners) unrelated to hats/caps/fashion before writing to cache.",
      changelog_li4_v170: "<span class=\"highlight-cyan\">Expanded Infinite Scroll:</span> Home feed supports endless loading, auto-fetching more results on scroll, with scraper priority pipeline: Cache → DuckDuckGo → Bing → Serper → Firecrawl.",
